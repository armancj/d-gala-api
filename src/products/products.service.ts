...
export class ProductsService {
  ...
  async create(createProductDto: CreateProductDto) {
    ...
    return this.prisma.product.create({data: {...createProductDto}}).catch((err) =>
        HandlerError(
            err,
  ...
  async findAll(getAllQueryDto: GetAllQueryDto) {
    const findCategory: GetAllResponseDto = {
      data: await this.prisma.product.findMany({where:{deleted: false}, skip: getAllQueryDto.skip, take: getAllQueryDto.take}),
  ...
  findOne(id: number) {
    return this.prisma.product.findFirstOrThrow({where: {id, deleted: false}}).catch((err) =>
        HandlerError(
            err,
  ...
  private async findOneProductName(name: string): Promise<void> {
    const productFounds = await this.prisma.product.findFirst({where: {name, deleted: false}});
    if(productFounds)
      throw new ConflictException('The product is duplicated')
  }
}
